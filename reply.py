...
#   ★★★0.70 → 1.20（2026-09-21）。★**20例の候補表を取って読んで決めた**。
#     ★0.70 だと平均 12.7本／最小 8本しか残らず、★切りすぎていた。
#     ★実測で変わったもの ── ★「おなかすいた」→ ★**そうなんですね。おなかすいたら…**
#       ★「今日は暑かったね」→ ★**暑かったね。でも出かけるって。**（★前は「梗かぁ。」）
#       ★「この前の話だけどさ」→ ★**この前の話とか、あれは最後です！**
#   ★★同じ20例で λ を下げる／長さ補正を切る も試したが、★**どちらも悪化**した。
#     ★λ↓ →「こんばんは→よろしくお願いします」と無難側へ戻る
#     ★長さ補正なし →「ありがとう→いきたいですね！」
FLOOR = float(os.environ.get("REALU_REPLY_FLOOR", 1.20))
