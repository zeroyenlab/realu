...
#   → ★凍る瞬間の偏りを浅くする。
#   ★★★【取り消し 2026-09-10 12:30】★上の診断は**間違い**だった。
#     ★見ていたのは★**層が増えた直後の一時的な落ち込み**で、★2回走ったら回復していた。
#     ★実測（09-10T02:05）: ★全体 **2.9351（初めて 3 を切った）**、
#       ★ビット/文字 **3.4704（過去最良）**、7ソース中6つが 7層の最良より良い。
#     → ★**0.5 に戻す。**★記録が出ている設定に触らない。
...
# ★【取り消し 2026-09-10 12:30】★一度 1.5 に下げたが、★その根拠だった診断が間違いだったので戻した。
...
def main():
    ...
    if pantry is None:
        ...
        for name in ("laws.txt", "wiki.txt", "aozora.txt", "talk.txt", "game.txt",
    ...
    if STEPS <= 0:
        ...
        if sec > 0 and remain > 0:
            # ★★★歩数は**多めに**見積もる（2026-09-10）。
            #   ★以前は 0.92 倍（少なめ）にしていた。★減衰まで走り切るため。
            #   ★★だが仕上げを**残り時間**で始めるようになったので、
            #     ★歩数を超過しても**必ず着地する**。★少なめにする理由が消えた。
            #   ★★★逆に少なすぎると損する: ★#65 は 3.06 秒/歩と見積もって 1,080 歩を組んだが、
            #     ★実際は **1.63 秒/歩**。★55分の予定が29分で終わり、★★**26分を捨てた**。
            #   ★GitHub の CPU の実測は 1.25〜3.16 秒/歩（★最大2.5倍のぶれ）。
            #   → ★★速ければ歩数が伸び、★遅ければ時間で仕上げに入る。★どちらに転んでも予算を使い切る。
            STEPS = max(300, int(remain * 1.8 / sec))
    ...
    for step in range(1, STEPS + 1):
        ...
        elif vl > best + WORSE_MARGIN:
            # ★★★2026-09-22: ★**詰まりと発散を分ける。**
            #   ★元は「良くならない＝詰まっている」として**一律に lr を上げて**いた。
            #   ★★実測（走行#331・L1）: ★前の自分 3.2930 →
            #     ★step100 3.4177（lr 1.8e-04）→ ★step200 3.5784（lr **2.4e-04**）。
            #     ★★**悪くなり続けているのに lr を上げ続けて**、さらに悪くしていた。
            #   ★体が大きくなって1回が900歩→200歩に減ったので、★★**立て直す余地が無い**。
            #   ★★★3系統とも「毎回ちゃんと学んで、毎回それを捨てる」状態になっていた
            #     （★L3 は33回連続で巻き戻し。★幅は元のまま＝体の大きさのせいではない）。
            #   → ★はっきり悪くなっている時は**逆に下げる**。★平らな時だけ上げる。
            bad += 1
            lr_now = max(LR_MIN, lr_now * FIRM)
        else:
            bad += 1
            lr_now = min(LR_MAX, lr_now * FRAGILE)     # ★平らなだけ → 壊れやすくする
        # ── ③★★★頭打ちなら大きくなる
